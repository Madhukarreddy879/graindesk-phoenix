defmodule RiceMillWeb.StockInLive.FormComponent do
  ...
  def render(assigns) do
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
      <.header>
        {@title}
        <:subtitle>Record a new paddy purchase from a farmer.</:subtitle>
      </.header>
      ...
      >
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <.input field={@form[:farmer_name]} type="text" label="Farmer Name" required />
          <.input field={@form[:farmer_contact]} type="text" label="Farmer Contact" required />
        </div>

        <.input field={@form[:vehicle_number]} type="text" label="Vehicle Number" required />
  ...
  @impl true
  def update(%{stock_in: stock_in} = assigns, socket) do
    changeset = Inventory.change_stock_in(stock_in)
  ...
  @impl true
  def handle_event("validate", %{"stock_in" => stock_in_params}, socket) do
    # Auto-fill price when product is selected
    stock_in_params = maybe_autofill_price(stock_in_params, socket.assigns.products)

    changeset =
      socket.assigns.stock_in
      |> Inventory.change_stock_in(stock_in_params)
  ...
  def handle_event("save", %{"stock_in" => stock_in_params}, socket) do
    save_stock_in(socket, socket.assigns.action, stock_in_params)
  end

  defp save_stock_in(socket, :new, stock_in_params) do
    tenant_id = socket.assigns.current_scope.user.tenant_id

    case Inventory.create_stock_in(tenant_id, stock_in_params) do
      {:ok, stock_in} ->
        # Preload product for display
        stock_in = RiceMill.Repo.preload(stock_in, :product)
        notify_parent({:saved, stock_in})

        {:noreply,
         socket
         |> put_flash(:info, "Stock-in entry created successfully")
